    ...
    <main className="flex min-h-screen flex-1 flex-col bg-surface">
      ...
      <div className="mx-auto w-full max-w-3xl flex-1 px-4 py-8">
        <div className="text-center">
          <h1 className="flex items-center justify-center gap-2 text-3xl font-bold text-brand-900">
            <Trophy size={28} className="text-brand-600" aria-hidden="true" />
            Leaderboard
          </h1>
            ...
            <Link
              ...
              className={`rounded-full px-4 py-1.5 text-sm font-semibold transition ${
                period.id === p.id
                  ...
                  : "bg-white text-stone-600 ring-1 ring-brand-200 hover:ring-brand-400"
              }`}
            >
              {p.label}
            </Link>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-brand-200 bg-white">
          ...
          {top.map((b, i) => (
            <div
              key={b.userId}
              className={`flex items-center gap-3 border-b border-brand-100 px-5 py-4 last:border-0 ${
                b.userId === user.id ? "bg-brand-50" : ""
              }`}
            >
              <span className="flex w-8 justify-center">
                {i < 3 ? (
                  ...
                  <span className="text-sm font-bold text-brand-500">{i + 1}</span>
                )}
              </span>
              ...
              <span className="flex items-center gap-1.5 font-bold text-brand-800">
                <Coins size={15} className="text-brand-500" aria-hidden="true" />
          ...
          {myRank !== null && myRank > 10 && (
            <div className="flex items-center gap-3 border-t-2 border-brand-200 bg-brand-50 px-5 py-4">
              <span className="flex w-8 justify-center text-sm font-bold text-brand-500">
                {myRank}
              </span>
              <span className="flex-1 font-semibold text-brand-900">
                You
                <span className="ml-2 rounded-full bg-brand-100 px-2 py-0.5 text-[11px] font-bold text-brand-700">
